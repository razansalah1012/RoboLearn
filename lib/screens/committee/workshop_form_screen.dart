...
class _WorkshopFormScreenState extends State<WorkshopFormScreen> {
  ...
  final List<String> _categories = ['Robotics', 'Programming', 'Drones', 'Engineering'];
  ...
  void initState() {
    ...
    _descriptionController = TextEditingController(text: widget.workshop?.description ?? '');
    _locationController = TextEditingController(text: widget.workshop?.location ?? '');
    _capacityController = TextEditingController(text: widget.workshop?.capacity.toString() ?? '');
    _imageUrlController = TextEditingController(text: widget.workshop?.imageUrl ?? '');
    _prerequisitesController = TextEditingController(text: widget.workshop?.prerequisites ?? '');
    _objectivesController = TextEditingController(text: widget.workshop?.objectives ?? '');
    _instructorController = TextEditingController(text: widget.workshop?.instructorName ?? '');
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_isEditMode ? "Workshop updated successfully" : "Workshop created successfully"),
  ...
  Future<void> _deleteWorkshop() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text("Delete Workshop", style: GoogleFonts.orbitron(fontWeight: FontWeight.bold)),
        content: const Text("Are you sure you want to delete this workshop? This action cannot be undone."),
        actions: [
    ...
    if (confirm == true && mounted) {
      ...
      try {
        ...
        if (mounted) {
          Navigator.pop(context);
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Workshop deleted"), backgroundColor: Colors.redAccent),
          );
        }
      } catch (e) {
        if (mounted) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(labelText: "Workshop Title*", prefixIcon: Icon(Icons.title)),
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: ListTile(
                            contentPadding: EdgeInsets.zero,
                            title: const Text("Reg. Deadline", style: TextStyle(fontSize: 12)),
                            subtitle: Text(DateFormat('yyyy-MM-dd').format(_registrationDeadline)),
                            trailing: const Icon(Icons.event_available, size: 20),
                      ...
                      children: [
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: const InputDecoration(labelText: "Max Capacity*", prefixIcon: Icon(Icons.people)),
                        ...
                        Expanded(
                          child: DropdownButtonFormField<WorkshopDifficulty>(
                            ...
                            items: WorkshopDifficulty.values.map((d) => DropdownMenuItem(
                              value: d,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(labelText: "Instructor Name", prefixIcon: Icon(Icons.person)),
                    ),
                    const SizedBox(height: 16),
                    TextFormField(
                      controller: _objectivesController,
                      decoration: const InputDecoration(labelText: "Learning Objectives", prefixIcon: Icon(Icons.list_alt)),
                      maxLines: 3,
                    ),
                    const SizedBox(height: 16),
                    TextFormField(
                      controller: _prerequisitesController,
                      decoration: const InputDecoration(labelText: "Prerequisites", prefixIcon: Icon(Icons.fact_check)),
                      maxLines: 2,
                    ),
                    const SizedBox(height: 16),
                    TextFormField(
                      controller: _imageUrlController,
                      decoration: const InputDecoration(labelText: "Poster URL", prefixIcon: Icon(Icons.image)),
                    ),
                  ],
                ),
              ),
